...
let numberOfItemsPerPage = items.selectedOptions[0].value;
...
let loadContent = () => {
    ...
    for(let i = 1; i <= getNumberOfPages(); i++) {
        pages.innerHTML = pages.innerHTML + `<a href="#" onClick="paginate(${i})" id="${i}" class="page-link">${i}</a>`;
    }
}
...
items.addEventListener('change', () => {
    numberOfItemsPerPage = items.selectedOptions[0].value;
    currentPageIndex = 1;
    loadContent();
    renderData(data);
})
...
function renderData(pageData) {
    ...
    let startIndex = (numberOfItemsPerPage * currentPageIndex) - numberOfItemsPerPage;
    let endIndex = numberOfItemsPerPage * currentPageIndex;

    let currentPageData = pageData.slice(startIndex, endIndex);
    renderCurrentPageData(currentPageData);
}

function renderCurrentPageData(currentPageData) {
...
function next() {
    if(currentPageIndex < getNumberOfPages()) {
        currentPageIndex++;
        renderData(data);
    }
}
...
function paginate(selectedIndex) {
